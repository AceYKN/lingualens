...
import { AlertTriangle, ArrowRight, Check, ChevronRight, CircleStop, Download, ExternalLink, FileUp, Heart, Languages, LoaderCircle, LockKeyhole, Menu, Settings2, SlidersHorizontal, Sparkles, WandSparkles, X } from 'lucide-react'
...
import { PRESETS, applyPreset } from './analysis/presets'
...
function App() {
  ...
  const [resultModules, setResultModules] = useState<Record<string, boolean> | null>(null)
  ...
  const analyze = async () => {
    ...
    setError(''); setResult(null); setResultModules(null); setLastUsage(undefined); setActiveSegment(null); setElapsedSeconds(0); setLoading(true)
    ...
    try {
      if (publicMode) {
        ...
        setResult(response.result); setResultModules(taskMode === 'analyze' ? { ...effectiveAnalysis.modules } : null); setLastUsage(response.usage); setPublicQuota(response.quota)
      } else {
        ...
        setResult(response.result); setResultModules(taskMode === 'analyze' ? { ...effectiveAnalysis.modules } : null); setLastUsage(response.usage)
      }
  ...
  const clearEverything = () => {
    ...
    clearAllData(); setAppConfig(defaultAppConfig()); setApiKey(''); setConnectionMode('public'); setText(''); setComparisonText(''); setTaskMode('analyze'); setResult(null); setResultModules(null); setLastUsage(undefined); setTestResult(null); setProviderOpen(false); setNotice('本地应用数据已清除')
  }
  ...
  return <div className="app-shell">
    ...
    <main id="top">
      <section className="intro-bar"><h1>读懂一句话，<em>从结构开始。</em></h1></section>

      <section className="workspace-grid">
        <div className="composer-card">
          <div className="card-heading"><div><span className="step-number">01</span><div><h2>{taskMode === 'analyze' ? '输入你想读懂的句子' : taskMode === 'correct' ? '输入你想检查的句子' : '输入两种想比较的表达'}</h2></div></div><span className="char-count">{text.length.toLocaleString()} / {inputLimit.toLocaleString()}</span></div>
          <div className="task-mode-tabs" role="tablist" aria-label="任务模式"><button className={taskMode === 'analyze' ? 'active' : ''} onClick={() => { setTaskMode('analyze'); setResult(null); setResultModules(null) }} role="tab" aria-selected={taskMode === 'analyze'}>理解句子</button><button className={taskMode === 'correct' ? 'active' : ''} onClick={() => { setTaskMode('correct'); setResult(null); setResultModules(null) }} role="tab" aria-selected={taskMode === 'correct'}>检查纠错</button><button className={taskMode === 'compare' ? 'active' : ''} onClick={() => { setTaskMode('compare'); setResult(null); setResultModules(null) }} role="tab" aria-selected={taskMode === 'compare'}>比较表达</button></div>
          <textarea className="sentence-input" dir={direction} value={text} onChange={(e) => { setText(e.target.value); setError(''); setResult(null); setResultModules(null); setLastUsage(undefined); setActiveSegment(null) }} maxLength={inputLimit + 1000} placeholder={taskMode === 'correct' ? '输入你自己写的句子…' : taskMode === 'compare' ? '表达 A…' : '粘贴或输入任意语言的句子…'} aria-label={taskMode === 'compare' ? '表达 A' : '待处理文本'} />
          {taskMode === 'compare' && <textarea className="sentence-input comparison-input" dir={direction} value={comparisonText} onChange={(e) => { setComparisonText(e.target.value); setError(''); setResult(null); setResultModules(null); setLastUsage(undefined) }} maxLength={inputLimit + 1000} placeholder="表达 B…" aria-label="表达 B" />}
          <div className="sample-row"><span>试试：</span>{(taskMode === 'correct' ? [{ label: '英语纠错', text: 'I very like it.' }, { label: '日语纠错', text: '私は昨日学校に行きます。' }] : samples).map((sample) => <button key={sample.label} onClick={() => { setText(sample.text); setResult(null); setResultModules(null); setLastUsage(undefined); setActiveSegment(null) }}>{sample.label}<ChevronRight size={13} /></button>)}</div>
          <div className="language-controls">
            ...
            <label className="field"><span>解释与翻译语言</span><select value={analysis.explanationLanguage} onChange={(e) => updateAnalysis({ ...analysis, explanationLanguage: e.target.value, translationLanguage: e.target.value })}>{EXPLANATION_LANGUAGES.map(([code, label]) => <option value={code} key={code}>{label}</option>)}</select></label>
          </div>
        ...
        <aside className="control-card">
          <div className="card-heading compact-heading"><div><span className="step-number">02</span><div><h2>{taskMode === 'analyze' ? '选择分析视角' : taskMode === 'correct' ? '本次检查会关注' : '本次比较会关注'}</h2></div></div></div>
          {taskMode === 'analyze' ? <><div className="preset-list">{(Object.entries(PRESETS) as Array<[Exclude<PresetId, 'custom'>, typeof PRESETS.quick]>).map(([id, item]) => <button className={analysis.preset === id ? 'active' : ''} key={id} onClick={() => selectPreset(id)}><span className="radio-mark" /><span><strong>{item.label}</strong><small>{item.description}</small></span>{id === 'standard' && <em>推荐</em>}</button>)}</div><button className={`custom-preset ${analysis.preset === 'custom' ? 'active' : ''}`} onClick={() => setAdvancedOpen(true)}><SlidersHorizontal size={18} /><span><strong>高级分析设置</strong><small>{analysis.preset === 'custom' ? '已自定义回答方式与内容' : '调整讲解方式、关注内容与重点'}</small></span><ChevronRight size={17} /></button></> : <div className="task-focus-card">{taskMode === 'correct' ? <><strong>准确 + 自然</strong><p>会区分错误与“虽正确但不够地道”的表达。</p></> : <><strong>逐项比较</strong><p>会说明两种表达在语义、语气和适用场景上的差别。</p></>}</div>}
          ...
          {loading ? <button className="analyze-button stop" onClick={() => requestController.current?.abort()}><CircleStop size={20} />取消请求</button> : <button className="analyze-button" onClick={analyze}><WandSparkles size={21} />{taskMode === 'correct' ? '开始检查' : taskMode === 'compare' ? '开始比较' : '开始分析'}<span><ArrowRight size={18} /></span></button>}
      ...
      {taskMode === 'analyze' && result && <section className="source-panel"><div className="source-panel-label"><span>原文</span>{activeSegment && <small>正在定位：{activeSegment.text}</small>}</div><SourceHighlight text={text} active={activeSegment} direction={direction} /></section>}
      ...
      {result && !isCorrectionResult(result) && !isComparisonResult(result) && <ResultView result={result} enabled={resultModules ?? analysis.modules} usage={lastUsage} onSegmentHover={setActiveSegment} />}
    ...
    <AdvancedSettings open={advancedOpen} publicMode={publicMode} config={analysis} onClose={() => setAdvancedOpen(false)} onApply={(next) => { updateAnalysis(next); setAdvancedOpen(false); setNotice('高级分析设置已应用') }} />
