...
import { AlertTriangle, ArrowRight, Check, ChevronRight, CircleStop, Download, FileUp, Languages, LoaderCircle, LockKeyhole, Menu, Settings2, ShieldCheck, SlidersHorizontal, Sparkles, WandSparkles } from 'lucide-react'
...
import { clearAllData, defaultAppConfig, loadSavedApiKey, loadSettings, parseImportedConfig, saveApiKey, saveSettings } from './storage/settings-storage'
...
function App() {
  ...
  const tokenEstimate = useMemo(() => estimateAnalysisTokens(text.trim(), effectiveAnalysis, provider.maxTokens), [text, effectiveAnalysis, provider.maxTokens])
  ...
  const analyze = async () => {
    ...
    if (trimmed.length > analysis.maxInputLength) { setError(`文本超过 ${analysis.maxInputLength.toLocaleString()} 字符限制，请缩短文本或在高级设置中调整。`); return }
    const invalid = validProvider(provider, apiKey)
    ...
    finally { setLoading(false); requestController.current = null }
  }

  const clearEverything = () => {
    if (!window.confirm('确定清除 API Key、已保存配置及当前页面数据吗？')) return
    clearAllData(); setAppConfig(defaultAppConfig()); setApiKey(''); setText(''); setResult(null); setLastUsage(undefined); setTestResult(null); setProviderOpen(false); setNotice('本地应用数据已清除')
  }
  ...
  return <div className="app-shell">
    <header className="topbar">
      ...
      <nav className="top-actions" aria-label="应用操作">
        ...
        <button className={`model-button ${apiKey ? 'connected' : ''}`} onClick={() => setProviderOpen(true)}><span className="status-dot" />{apiKey ? provider.model : '配置模型'}<Settings2 size={16} /></button>
    ...
    <main id="top">
      <section className="intro-bar">
        ...
        <div className="intro-trust"><span><ShieldCheck size={16} />纯前端直连</span><small>文本与 Key 不经过项目方服务器</small></div>
      </section>

      {privacyOpen && <aside className="privacy-strip"><ShieldCheck size={20} /><div><strong>隐私优先</strong><span>你的文本会直接发送到所选 LLM 服务。请勿输入密码、证件号码等敏感信息；第三方服务会按其隐私政策处理数据。</span></div><button onClick={() => setPrivacyOpen(false)}>我知道了</button></aside>}

      <section className="workspace-grid">
        <div className="composer-card">
          <div className="card-heading"><div><span className="step-number">01</span><div><h2>输入你想读懂的句子</h2><p>支持多语言、混合语言与 Unicode 标点</p></div></div><span className="char-count">{text.length.toLocaleString()} / {analysis.maxInputLength.toLocaleString()}</span></div>
          <textarea className="sentence-input" dir={direction} value={text} onChange={(e) => { setText(e.target.value); setError('') }} maxLength={analysis.maxInputLength + 1000} placeholder="粘贴或输入任意语言的句子…" aria-label="待分析文本" />
        ...
        <aside className="control-card">
          ...
          <div className="model-summary"><span className="status-dot" /><div><small>当前模型</small><strong>{apiKey ? provider.model : '尚未配置'}</strong></div><button onClick={() => setProviderOpen(true)}>设置</button></div>
          ...
          <p className="cost-note"><LockKeyhole size={13} />长文本与深度分析会增加 API 消耗</p>
        </aside>
      </section>

      {error && <div className="error-banner" role="alert"><AlertTriangle size={20} /><span>{error}</span><button onClick={() => setError('')}>关闭</button></div>}
      {loading && <section className="loading-state"><div className="analysis-loader"><LoaderCircle className="spin" size={28} /></div><div><h3>正在辨认结构与语义关系…</h3><p>请求直接发往 {provider.baseUrl.replace(/^https?:\/\//, '').split('/')[0]}</p></div></section>}
    ...
    <footer><span>语镜 LinguaLens · 数据不经过项目方服务器</span><button onClick={() => setPrivacyOpen(true)}>隐私与安全说明</button><span>AI 分析仅供学习参考</span></footer>
    {notice && <div className="toast" role="status"><Check size={17} />{notice}</div>}
    <ProviderDrawer open={providerOpen} config={provider} apiKey={apiKey} testing={testing} testResult={testResult} onClose={() => setProviderOpen(false)} onConfigChange={updateProvider} onApiKeyChange={setApiKey} onTest={testConnection} onClearKey={() => { setApiKey(''); saveApiKey(''); setTestResult(null); setNotice('API Key 已清除') }} onClearAll={clearEverything} />
    <AdvancedSettings open={advancedOpen} config={analysis} onClose={() => setAdvancedOpen(false)} onChange={updateAnalysis} onReset={() => { updateAnalysis(DEFAULT_ANALYSIS); setNotice('分析设置已恢复默认') }} />
  </div>
}
